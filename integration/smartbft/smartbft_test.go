...
var _ = Describe("EndToEnd Smart BFT configuration test", func() {
	...
	Describe("smartbft network", func() {
		...
		It("smartbft deliverer", func() {
			...
			networkConfig := nwo.MultiNodeSmartBFT()
			networkConfig.Channels = nil
			//networkConfig.Orderers = networkConfig.Orderers[:1]
			channel := "testchannel1"
			//networkConfig.Peers = networkConfig.Peers[:1]
			...
			go func() {
				...
				for {
					...
					//By(fmt.Sprintf(">>> Sending env #%d", i))
			...
			for _, orderer := range network.Orderers {
				...
				ms, err := NewMockServer(ledgerArray, nil, cert, key, [][]byte{rootCaCert}, network.OrdererAddress(orderer, nwo.ListenPort))
				ms.logger.Info("")
			}
